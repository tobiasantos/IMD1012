...
int completaTabuleiro(char **m, int quant, int *posX, int *posY, int aux) {
  int x, y;
  if (quant-aux == 0) {
    return 1;
  } else {
    for (int i=1; i<10; i++) {
      ...
      if (!repeteSubMatriz(m, x, y, i+'0') && !repeteLinha(m, x, y, i+'0') && !repeteColuna(m, x, y, i+'0')) {
        m[x][y] = i + '0';
        if (!completaTabuleiro(m, quant, posX, posY, aux+1)) {
            m[x][y] = '*';
        } else {
          return 1;
        }
...
int main() {
  ...
  if (completaTabuleiro(matriz, quantVazios, posX, posY, 0)) {
